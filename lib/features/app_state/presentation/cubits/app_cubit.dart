...
class AppCubit extends Cubit<AppStateEntity> {
  ...
  Future<void> updateJarWalletSources({
    ...
    final linkedWallets = state.budgetSetup.linkedWallets
        .map((j) => j.id == jarId ? j.copyWith(walletSources: sources) : j)
        .toList();
    final next = state.copyWith(
    ...
    await _applyAndLog(
      ...
      entityType: 'jar',
      entityId: jarId,
      details: 'تم تحديث مصادر الحصالة',
      apply: () async => next,
    );
  }

  /// تحويل داخلي بين حصالتين — يغير الأرصدة الافتراضية والـ labels
  /// بدون أي transaction فعلية وبدون تغيير أرصدة المحافظ
  Future<void> transferBetweenJars({
    ...
    final src = linkedWallets[srcIdx];
    final tgt = linkedWallets[tgtIdx];

    // Update source: decrease balance + decrease walletSource label
    List<JarWalletSource> srcSources = List.from(src.walletSources);
    if (physicalWalletId != null) {
      final existing = srcSources.firstWhere(
        (s) => s.walletId == physicalWalletId,
    ...
    linkedWallets[srcIdx] = src.copyWith(
      ...
      walletSources: srcSources,
    );

    // Update target: increase balance + increase walletSource label
    List<JarWalletSource> tgtSources = List.from(tgt.walletSources);
    if (physicalWalletId != null) {
      final existing = tgtSources.firstWhere(
        (s) => s.walletId == physicalWalletId,
        orElse: () => JarWalletSource(walletId: physicalWalletId, amount: 0),
      );
      final newAmt = existing.amount + amount;
      tgtSources = [
        ...tgtSources.where((s) => s.walletId != physicalWalletId),
        JarWalletSource(walletId: physicalWalletId, amount: newAmt),
      ];
    }
    linkedWallets[tgtIdx] = tgt.copyWith(
      balance: tgt.balance + amount,
      walletSources: tgtSources,
    );

    final srcName = src.name;
    final tgtName = tgt.name;
    final next = state.copyWith(
      budgetSetup: state.budgetSetup.copyWith(linkedWallets: linkedWallets),
    );
    await _applyAndLog(
